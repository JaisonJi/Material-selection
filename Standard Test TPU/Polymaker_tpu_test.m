close all;
clear all;
name=['Polymaker_tpu_1.csv'];
data=xlsread(name);
N=4505;
n=2;
A0=0.0032*0.006;
for i=1:6
    A1(:,2*i-1:2*i)=data(N*(i-1)+n*i+1:N*i+n*i,3:4);
end
L01=[33 33 33 33.5 33 33];
L02=[40.5 40.5 41 41 40.5 40.5];
R=(L02-L01)/15;
a = 1;
b = ones(1,64)./64;
figure
for i=1:6
    x(:,i)=R(i).*A1(:,2*i-1)./L01(i);
    z(:,i)=A1(:,2*i)./A0;
    y(:,i)=filter(b,a,z(:,i));
    plot(x(:,i),y(:,i));
    hold on
